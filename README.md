...
stub. For real model calls, add a `.env` file in `backend/`:
...
available). The frontend reads `REACT_APP_API_URL` to point at a non-local
backend; it defaults to `http://localhost:8000`.
...
```mermaid
flowchart LR
  subgraph FE["Frontend — React + React Flow"]
    direction TB
    UI["Canvas & edge styling<br/>(ui.js)"]
    ABS["Node abstraction<br/>BaseNode · NodeField · nodeConfigs"]
    STORE["State + persistence<br/>(zustand store)"]
    UI --- STORE
    ABS --- STORE
  end
  ...
  subgraph BE["Backend — FastAPI"]
    direction TB
    PARSE["POST /pipelines/parse<br/>DAG check"]
    RUN["POST /pipelines/run<br/>execute"]
    GEN["POST /pipelines/generate<br/>prompt → graph"]
    SAVE["POST /pipelines + /{id}/run<br/>deploy"]
    ENG["engine.py<br/>topological execution"]
    RUN --> ENG
    SAVE --> ENG
  end
  ...
  STORE -->|"nodes + edges"| PARSE
  STORE -->|"nodes + edges + inputs"| RUN
  STORE -->|"prompt"| GEN
  ENG -->|"LLM node"| OPENROUTER
  GEN -->|"draft spec"| OPENROUTER
```
...
The Pydantic models accept extra fields (`ConfigDict(extra='allow')`) and pin only
what the parse depends on — `id` on a node, `source`/`target` on an edge. React
Flow nodes carry a lot the backend doesn't care about (position, width,
`selected`, `dragging`), and that shape changes as the frontend evolves. Modeling
only the contract means a new frontend field can't break the backend.
...
[`nodes/nodeConfigs.js`](frontend/src/nodes/nodeConfigs.js). There are no per-node
files. Adding a node means adding a config — no new component, no duplicated
shell, no hand-placed handles:
...
That config alone produces a styled node on the canvas, a draggable palette entry
(filed under its `category`), correctly-spaced handles, and field edits written
through to the store. Three pieces make it work:
...
| [`NodeField.js`](frontend/src/nodes/NodeField.js) | A field registry — `text`, `select`, `number`, `checkbox`, `textarea`, `autosize-textarea` |
...
- **`BaseNode` spaces handles itself** — `n` handles on an edge land at
  `100·(i+1)/(n+1)`%. Configs never compute pixel offsets. A `render()` escape
  hatch covers the rare node that needs bespoke markup without leaving the shell.

**The five new nodes** (`math`, `filter`, `apiRequest`, `merge`, `note`) were
chosen to stress the abstraction: between them they exercise every field type, the
`render()` hatch, a node with *no* handles, and a dynamic handle count driven by a
field (`merge`).
...
The organizing idea: **cool slate neutrals carry the interface, so color only ever
means something.** A hue on screen is either an action or a node's category
(`io` sky, `ai` indigo, `logic` amber, `core` slate). Edge appearance is decided
in [`ui.js`](frontend/src/ui.js) at render time and nowhere else, so a generated
edge and a hand-drawn one can't drift apart visually.
...
- **Variables.** [`textVariables.js`](frontend/src/nodes/textVariables.js) parses
  `{{ name }}` placeholders — valid JS identifiers only, deduped, order preserved —
  and each becomes a target handle.

This is the payoff from Part 1's dynamic handles: the mechanism built for
`merge`'s input count is **reused verbatim** here. When a `{{variable}}` is
deleted, its handle disappears and the store's `pruneEdgesForNode` drops any edge
left dangling.
...
are handled deliberately — a self-loop gives a node an in-degree it can never shed
so it falls out naturally; an empty pipeline is vacuously acyclic; an edge with an
endpoint off the canvas counts toward `num_edges` but is skipped when walking.
...
[`engine.py`](backend/engine.py) walks the graph in topological order (reusing the
same Kahn sort) and executes each node for real: Input emits its value, Text
...
Every node runs behind a guard — a node that raises records the error on itself
and yields empty rather than sinking the run. The LLM node degrades to a labelled
stub when there's no API key or the call fails, so a "Run" click never dead-ends.
...
filling defaults, and repairing the two things models reliably get wrong (a
`{{variable}}` referenced by an edge but missing from the text; a dangling Output
node with nothing wired in). Ask the model for judgment, not bookkeeping. A
malformed spec raises a clean 400 and never corrupts the canvas.
...
- **Cycle highlighting** — [`graph.js`](frontend/src/graph.js) runs the same Kahn
  ...
  than after a Submit round-trip.
- **Connection validation** — `isValidConnection` blocks self-loops, duplicate
  wires, and a File-typed Input feeding a text-consuming node.
- **Persistence** — zustand's `persist` middleware; a refresh doesn't nuke your work.
- **Templates** — three real, runnable pipelines on the empty canvas.
...
- **CORS** allows `localhost:3000` by default; set `FRONTEND_URL` (comma-separated
  for multiple) to allow a deployed frontend origin.
